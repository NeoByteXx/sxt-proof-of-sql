...
contract GroupByExecTest is Test {
    function testSimpleGroupByExec() public pure {
        bytes memory plan = abi.encodePacked(
            ...
            abi.encodePacked(LITERAL_EXPR_VARIANT, LITERAL_BIGINT_VARIANT, int64(102)), // group_by_expr[0]
            abi.encodePacked(LITERAL_EXPR_VARIANT, LITERAL_BIGINT_VARIANT, int64(103)), // group_by_expr[1]
            uint64(2), // sum_count
            abi.encodePacked(LITERAL_EXPR_VARIANT, LITERAL_BIGINT_VARIANT, int64(104)), // sum_expr[0]
            abi.encodePacked(LITERAL_EXPR_VARIANT, LITERAL_BIGINT_VARIANT, int64(105)), // sum_expr[1]
    ...
    function testFuzzGroupByExec(
        ...
        plan = abi.encodePacked(plan, abi.encodePacked(LITERAL_EXPR_VARIANT, LITERAL_BIGINT_VARIANT, where));

        // Add group_by expressions
        plan = abi.encodePacked(plan, uint64(g_in_vals.length));
        for (uint256 i = 0; i < g_in_vals.length; i++) {
            plan = abi.encodePacked(plan, abi.encodePacked(LITERAL_EXPR_VARIANT, LITERAL_BIGINT_VARIANT, g_in_vals[i]));
        }
        ...
        for (uint256 i = 0; i < sum_in_vals.length; i++) {
            plan =
                abi.encodePacked(plan, abi.encodePacked(LITERAL_EXPR_VARIANT, LITERAL_BIGINT_VARIANT, sum_in_vals[i]));
        }
